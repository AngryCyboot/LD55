...
func step(foot_target : Vector3,delta : float) -> void:
	...
	if dist > close_enough or global_position.y > close_enough/2:
		...
		var rotation_radius : float = boss.global_position.distance_to(global_position)
...
func mobile(x : bool)-> void:
	if x != is_mobile:
		is_mobile = x
		visual.set_instance_shader_parameter("mobile",is_mobile)
		if is_mobile :
			parent.mobile += 1
		else:
			parent.mobile -= 1
